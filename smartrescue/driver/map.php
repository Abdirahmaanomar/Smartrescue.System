...
$type_map = [
    ...
    'police'   => ['icon' => 'fa-shield-halved',     'color' => '#6366f1'],
...
function makeDriverIcon() {
    return L.divIcon({
        className:'',
        html:`<div style="width:44px;height:44px;border-radius:50%;background:#2563eb;display:flex;align-items:center;justify-content:center;box-shadow:0 0 20px #2563eb99;border:3px solid #fff;color:#fff;font-size:18px"><i class="fa-solid fa-truck-medical"></i></div>`,
        iconSize:[44,44],iconAnchor:[22,22]
    });
}
function makeVictimIcon() {
    return L.divIcon({
        className:'',
        html:`<div style="width:46px;height:46px;border-radius:50%;background:#ef4444;display:flex;align-items:center;justify-content:center;box-shadow:0 0 22px #ef444499;border:3px solid #fff;color:#fff;font-size:18px"><i class="fa-solid fa-person-falling-burst"></i></div>`,
...
function _placeVictimOnMap(vLat, vLng) {
    if (!victimMarker) { victimMarker = L.marker([vLat,vLng],{icon:makeVictimIcon()}).addTo(gMap); }
    else victimMarker.setLatLng([vLat,vLng]);
...
setInterval(() => {
    if (victimMarker) {
        ...
        _placeVictimOnMap(vll.lat, vll.lng);
    }
}, 30000);
