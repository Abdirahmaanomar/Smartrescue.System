...
// Fetch Real Logs for the Dashboard
...
// 3. Marka la save-gareeyo
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['ajax']) && $_POST['ajax'] === '1') {
        
        // Settings common to all panels
        $excluded = ['ajax', 'new_password'];
        
        // Define all checkboxes to handle the 'missing from POST' issue
        $checkboxes = ['notif_email', 'notif_sms', 'notif_sound', 'auto_backup'];
        ...
        // Handle checkboxes that were unchecked (and thus missing from $_POST)
        foreach($checkboxes as $cb) {
            if(!isset($_POST[$cb])) {
                update_setting($conn, $cb, '0');
            } else {
                update_setting($conn, $cb, '1'); // Force standardizing to 1/0
            }
        }
        ...
        if(!empty($_POST['new_password'])) {
            ...
            mysqli_query($conn, "UPDATE users SET password = '$new_pass' WHERE role = 'admin'");
        }

        // Log activity
        log_activity($conn, $_SESSION['user_id'], 'Settings Updated', 'Admin modified system configuration parameters.', 'success');

        echo json_encode(['status' => 'success', 'message' => 'System parameters synchronized successfully.', 'reload' => true]);
        exit();
    }
}
...
.avatar-upload-zone { text-align: center; margin-bottom: 24px; }
.avatar-preview { width: 100px; height: 100px; border-radius: 50%; border: 3px solid var(--surface); box-shadow: var(--shadow-md); object-fit: cover; margin-bottom: 12px; }
.avatar-btn { display: inline-flex; align-items: center; gap: 6px; font-size: 0.8rem; font-weight: 700; color: var(--primary); background: var(--primary-light); padding: 6px 14px; border-radius: 20px; cursor: pointer; transition: all 0.2s; }
...
<main class="main-wrapper">
    <?php $page_title = 'System Configuration'; $page_subtitle = 'Admin Dashboard Settings'; include 'includes/topbar.php'; ?>
    ...
    <div class="status-dashboard">
        ...
        <div class="status-pill">
            <div class="status-icon si-api"><i class="fa fa-bolt"></i></div>
            <div class="status-info">
                <h4>124ms <small>Avg</small></h4>
                <p>API Latency</p>
            </div>
        </div>
        <div class="status-pill">
                ...
                <div class="settings-panel active" id="panel-general">
                    <div class="s-card">
                        <div class="s-card-header">
                            ...
                            <div>
                                <h3 class="sc-title"><?= t('General Settings') ?></h3>
                                <p class="sc-desc"><?= t('Core application details used across the interface.') ?></p>
                            </div>
                        </div>
                        <div class="form-row">
                ...
                <div class="settings-panel" id="panel-notify">
                    <div class="s-card">
                        <div class="s-card-header">
                            ...
                            <div>
                                <h3 class="sc-title"><?= t('Notification Delivery') ?></h3>
                                <p class="sc-desc"><?= t('Control automated messages and dispatch alerts.') ?></p>
                            </div>
                        </div>
                        
                        <div class="setting-toggle-row">
                            <div class="str-info">
                                <h6><?= t('Email Dispatch Summary') ?></h6>
                                <p><?= t('Send full incident reports to admin email upon resolution.') ?></p>
                            </div>
                            <label class="s-toggle">
                                <input type="checkbox" name="notif_email" class="track-change" <?= ($settings['notif_email'] == '1' || $settings['notif_email'] == 'on') ? 'checked' : '' ?>>
                                <span class="s-toggle-slider"></span>
                            </label>
                        </div>
                        <div class="setting-toggle-row">
                            <div class="str-info">
                                <h6><?= t('Unit SMS Dispatch') ?></h6>
                                <p><?= t('Text the ambulance driver instantly when assigned to a mission.') ?></p>
                            </div>
                            <label class="s-toggle">
                                <input type="checkbox" name="notif_sms" class="track-change" <?= ($settings['notif_sms'] == '1' || $settings['notif_sms'] == 'on') ? 'checked' : '' ?>>
                                <span class="s-toggle-slider"></span>
                            </label>
                        </div>
                        <div class="setting-toggle-row">
                            <div class="str-info">
                                <h6><?= t('Admin Browser Sounds') ?></h6>
                ...
                <div class="settings-panel" id="panel-system">
                    <div class="s-card">
                        <div class="s-card-header">
                            ...
                            <div>
                                <h3 class="sc-title"><?= t('Instance Configuration') ?></h3>
                                <p class="sc-desc"><?= t('Server-level parameters and synchronization behaviors.') ?></p>
                            </div>
                        </div>
                ...
                <div class="settings-panel" id="panel-backup">
                    <div class="s-card mb-4">
                        <div class="s-card-header">
                            ...
                            <div>
                                ...
                                <p class="sc-desc"><?= t('Manage data lifecycle and archival.') ?></p>
                            </div>
                        </div>
                        ...
                        <div class="form-row mt-4">
                            ...
                            <p class="form-hint"><?= t('Delete old incidents/tracking logs older than...') ?></p>
                        </div>

                        <div style="margin-top:20px; padding:16px; background:#f8fafc; border:1px dashed var(--border); border-radius:var(--radius-md); text-align:center;">
                            <i class="fa fa-file-zipper" style="font-size:2rem; color:var(--text-light); margin-bottom:12px;"></i>
                            <p style="font-size:0.8rem; font-weight:600; color:var(--text-main); margin-bottom:12px;">Hot Snapshot: Manual Restore Point</p>
                            <a href="../api/admin/generate_backup.php" class="btn-primary" style="text-decoration:none;"><i class="fa fa-download me-2"></i> Download Latest SQL</a>
                        </div>
                    </div>
                    ...
                    <div class="s-card">
                        <div class="s-card-header">
                            ...
                            <div>
                                <h3 class="sc-title">Recent System Activity</h3>
                                <p class="sc-desc">Logs of key changes for auditing purposes.</p>
                            </div>
                        </div>
                        <ul class="activity-list">
                            <?php if (empty($logs)): ?>
                                <li style="text-align:center; padding:20px; color:var(--text-muted); font-size:0.85rem;">No recent activities logged.</li>
                            <?php endif; ?>
...
<div class="sticky-save-bar" id="saveBar">
    <div class="ssb-info">
        ...
        <div class="ssb-text">
            <h5>Unsaved Changes Detected</h5>
            <p>You have modified system parameters. Save to apply changes broadly.</p>
        </div>
    </div>
    <div class="ssb-actions">
        <button class="btn-outline" onclick="discardChanges()">Discard</button>
        <button class="btn-primary" id="saveBtn" onclick="saveSettings()">
            <i class="fa fa-cloud-arrow-up"></i> Apply Settings
        </button>
    </div>
</div>
...
navItems.forEach(item => {
    item.addEventListener('click', () => {
        // Remove active class from all navs and panels
    ...
    fetch('settings.php', { method: 'POST', body: fd })
        ...
        .then(data => {
            if(data.status === 'success') {
                ...
                // Reload page after short delay so language/settings take effect
                if (data.reload) {
        ...
        .finally(() => {
            btn.innerHTML = '<i class="fa fa-cloud-arrow-up"></i> Apply Settings';
